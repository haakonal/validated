...
def run_scenario(title: str, telemetry: SatelliteTelemetry):
    ...
    print(
        f"  • {BOLD}Power & Battery:{RESET} charge={BOLD}{bat.charge_level}%{RESET} | "
    ...
    acs = telemetry.acs
    print(
        f"  • {BOLD}ACS State:{RESET} pointing_dev={BOLD}{acs.pointing_deviation}°{RESET} | "
...
def main():
    ...
    negative_net_power_charging = SatelliteTelemetry(
        ...
        battery=BatteryState(
            charge_level=75.0, status="charging", temperature=22.0, current_draw=120.0
        ),  # drawing 120W
        panels=[
    ...
    overheating_data = SatelliteTelemetry(
        mode="data_collection",
        battery=BatteryState(
            charge_level=85.0, status="discharging", temperature=45.0, current_draw=90.0
        ),  # 45°C > 40°C limit
        panels=[
            SolarPanelState(deployed=True, power_generated=10.0),
            SolarPanelState(deployed=True, power_generated=10.0),
    ...
    multiple_failures_charging = SatelliteTelemetry(
        mode="charging",
        battery=BatteryState(
            charge_level=40.0, status="discharging", temperature=22.0, current_draw=60.0
        ),  # low charge level, wrong status
        panels=[
            SolarPanelState(deployed=True, power_generated=10.0),
            SolarPanelState(
                deployed=False, power_generated=0.0
            ),  # One folded panel (total 10W gen < 60W draw = negative net power)
        ],
        acs=ACSState(
            reaction_wheel_speeds=np.array([1200.0, -800.0, 450.0], dtype=np.float64),
            momentum_wheel_speed=3000.0,
    ...
    try:
        print("Attempting to construct healthy SlewTask (Sydney_Station, speed=1.2, coverage=88.5%)...")
        slew_ok = SlewTask(
        ...
        print(
            f"Validating SlewTask to {BOLD}{slew_ok.poi_name}{RESET} (speed={slew_ok.max_predicted_slew_speed} deg/s, "
            f"coverage={slew_ok.predicted_coverage}%)"
        )
        print(
    ...
    try:
        print(
            f"Attempting to construct SlewTask with {BOLD}speed=3.5 AND coverage=75.0%{RESET} (both violate limits)..."
        )
        SlewTask(
            poi_name="Sydney_Station",
            target_yaw=45.2,
